...
for i in range(num_episodes):
    ...
    while not done:
        action = np.argmax(Q_1[state, :] + Q_2[state, :] + Q_3[state, :] + Q_4[state, :] + np.random.randn(1, env.action_space.n) * (1. / (i + 1)))
        new_state, reward, done, _ = env.step(action)
        x = np.random.rand()
        if x > 0.75:
            Q_1[state, action] = Q_1[state, action] + alpha * (reward + gamma * ((QS[1] + QS[2] + QS[3]) / (len(QS) - 1))[new_state, np.argmax(Q_1[new_state, :])] - Q_1[state, action])
        elif x > 0.50:
