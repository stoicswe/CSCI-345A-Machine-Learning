...
W2 = tf.Variable(tf.truncated_normal([2,2]), name = "W2")
w2 = tf.Variable(tf.truncated_normal([2,1]), name = "w2")

W3 = tf.Variable(tf.truncated_normal([2,2]), name = "W3")
w3 = tf.Variable(tf.truncated_normal([2,1]), name = "w3")

W4 = tf.Variable(tf.truncated_normal([2,2]), name = "W4")
w4 = tf.Variable(tf.truncated_normal([2,1]), name = "w4")

c = tf.Variable(tf.zeros([887,2]), name = "c")
b = tf.Variable(tf.zeros([887,1]), name = "b")

c1 = tf.Variable(tf.zeros([887,2]), name = "c1")
c2 = tf.Variable(tf.zeros([887,2]), name = "c2")
c3 = tf.Variable(tf.zeros([887,2]), name = "c3")
c4 = tf.Variable(tf.zeros([887,2]), name = "c4")

with tf.name_scope("hidden_layer") as scope:
    h = tf.nn.relu(tf.add(tf.matmul(X, W),c))
    h1 = tf.nn.relu(tf.add(tf.matmul(h, W1),c1))
    h2 = tf.nn.relu(tf.add(tf.matmul(h1, W2),c2))
    h3 = tf.nn.relu(tf.add(tf.matmul(h2, W3),c3))
    h4 = tf.nn.relu(tf.add(tf.matmul(h3, W4),c4))

with tf.name_scope("output") as scope:
    y_estimated = tf.sigmoid(tf.add(tf.matmul(h4,w4),b))
