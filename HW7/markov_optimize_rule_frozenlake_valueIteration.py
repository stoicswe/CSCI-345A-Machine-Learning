...
def value_iteration(env, gamma = 0.9):
    ...
    print("Number of iterations to policy: %d\n" %k)
    return v

def policy_improvement(policy, V, env):
    envUp = env.env
...
if __name__ == "__main__":
    ...
    while stable_policy == False:
        v = value_iteration(envUp)#policy_evaluation(policy, env, gamma)
        new_policy, num_changes = policy_improvement(policy, v, env)
